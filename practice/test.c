...
int main(){
  ...
  while(1){
	...
	for(int j=0;j<4;j++){
	  ...
	  // if(checkTeam(groups[i].team[j],groups,cur)==0){
	...
	for(int j=0;j<4;j++){
	  printf("%-20s%-10d%-10d\n",groups[i].team[j],groups[i].point[i],groups[i].goals[j]);
	}
      }
      break; 
    }
    ...
    if(temp1!=temp2){
      ...
      groups[i1].goals[j1]=first;
      groups[i2].goals[j2]=second;
       printf("Group %c\n",groups[i1].ID);
	printf("Team                Point     Goal\n");
	for(int j=0;j<4;j++){
	  printf("%-20s%-10d%-10d\n",groups[i1].team[j],groups[i1].point[i1],groups[i1].goals[j]);
	}
    }
